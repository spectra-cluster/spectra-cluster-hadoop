...
hadoop jar spectra-cluster-hadoop-1.0.1-SNAPSHOT.jar uk.ac.ebi.pride.spectracluster.hadoop.spectrum.SpectrumToClusterJob -libjars ${LIB_JARS} -conf ${HADOOP_CONF} "SPECTRUM_TO_CLUSTER${JOB_PREFIX}" "${JOB_CONF}/spectrum-to-cluster.xml" ${SPECTRUM_TO_CLUSTER_COUNTER_FILE} ${SPECTRUM_TO_CLUSTER_DIR} ${INPUT_DIR}
...
hadoop jar spectra-cluster-hadoop-1.0.1-SNAPSHOT.jar uk.ac.ebi.pride.spectracluster.hadoop.peak.MajorPeakJob -libjars ${LIB_JARS} -conf ${HADOOP_CONF} "MAJOR_PEAK${JOB_PREFIX}" "${JOB_CONF}/major-peak.xml" ${MAJOR_PEAK_COUNTER_FILE} ${MAJOR_PEAK_DIR} ${SPECTRUM_TO_CLUSTER_DIR} ${PREVIOUS_RESULT_DIR}
...
hadoop jar spectra-cluster-hadoop-1.0.1-SNAPSHOT.jar uk.ac.ebi.pride.spectracluster.hadoop.merge.MergeClusterByIDJob -libjars ${LIB_JARS} -conf ${HADOOP_CONF} "MERGE_BY_ID${JOB_PREFIX}" "${JOB_CONF}/merge-by-id.xml" ${MERGE_BY_ID_COUNTER_FILE} ${MERGE_BY_ID_DIR} ${MAJOR_PEAK_DIR}
...
hadoop jar spectra-cluster-hadoop-1.0.1-SNAPSHOT.jar uk.ac.ebi.pride.spectracluster.hadoop.merge.MergeClusterJob -libjars ${LIB_JARS} -conf ${HADOOP_CONF} "MERGE_CLUSTER_BY_OFFSET${JOB_PREFIX}" "${JOB_CONF}/merge-cluster-by-offset.xml" ${MERGE_BY_OFFSET_COUNTER_FILE} ${MERGE_BY_OFFSET_DIR} ${MERGE_BY_ID_DIR}
...
# execute merge job
hadoop jar spectra-cluster-hadoop-1.0.1-SNAPSHOT.jar uk.ac.ebi.pride.spectracluster.hadoop.merge.MergeClusterJob -libjars ${LIB_JARS} -conf ${HADOOP_CONF} "MERGE_CLUSTER${JOB_PREFIX}" "${JOB_CONF}/merge-cluster.xml" ${MERGE_COUNTER_FILE} ${MERGE_DIR} ${MERGE_BY_OFFSET_DIR}
...
# execute output job
hadoop jar spectra-cluster-hadoop-1.0.1-SNAPSHOT.jar uk.ac.ebi.pride.spectracluster.hadoop.output.OutputClusterJob -libjars ${LIB_JARS} -conf ${HADOOP_CONF} "OUTPUT_CLUSTER${JOB_PREFIX}" "${JOB_CONF}/output-cluster.xml" ${OUTPUT_COUNTER_FILE} ${OUTPUT_DIR} ${MERGE_DIR}
